...
func StartMCPServer(ctx context.Context) error {
	...
	for _, mcpServerInfo := range mcpServerList.List {
		if isManagedCampusMCP(mcpServerInfo.McpServerId) {
			continue
		}
...
func toMCPServerInfo(ctx *gin.Context, mcpServerInfo *mcp_service.MCPServerInfo) response.MCPServerInfo {
	managed := isManagedCampusMCP(mcpServerInfo.McpServerId)
...
func isManagedCampusMCP(id string) bool {
	switch id {
	case campusstudent.MCPCode, campusbusiness.TeacherMCPCode, campusbusiness.AcademicMCPCode:
		return true
	default:
		return false
	}
